 ...
 */
public class SlowCreator implements AsteroidCreator {
    @Override
    public Asteroid createAsteroid(PApplet app) {
        PVector dirct = PVector.random2D();
        Vector2 direction = new Vector2(dirct.x, dirct.y);
        Asteroid asteroid = new Asteroid(new Vector2((float)Math.random() * app.width,(float) Math.random() * app.height), app, direction, 1);
        return asteroid;
    }
}
